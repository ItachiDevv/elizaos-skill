...
description: >
  ...
  latest tag `v2.0.3-beta.5` as of 2026-06-26), capability tiers (Basic/Extended/Autonomy),
  ServiceBuilder API, x402 payments, and Dexter SDK integration. Legacy v1 (1.4.4) lives on the `main` branch.
  Last verified against the live repo: 2026-08-10.
---
...
### Branches & Versions (verified 2026-08-10)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.4` | Active development — this is the v2 line | 23 packages, 130 plugins |
...
**Latest published tag:** `v2.0.3-beta.5` (2026-06-26). Beta releases ship from `develop` — install via `bun i -g @elizaos/cli@beta`.
...
### Monorepo Layout — `develop` branch (verified 2026-08-10)
...
packages/
  ...
  evidence/           → Evidence and verification system
  examples/           → Example agents
  homepage/           → elizaos.ai marketing site
  logger/             → Logging package
  native/             → Built-in native plugins (sql, bootstrap, etc.)
  ...
  test/               → Test utilities
  training/           → Training and fine-tuning support
  ui/                 → React web dashboard
  vault/              → Secrets and vault management

plugins/              → 130 first-party plugins at top level (Discord,
                        Telegram, Twitter, Slack, OpenAI, Anthropic,
                        browser, calendar, GitHub, MCP, local inference,
                        native mobile plugins, and many more)
```
...
- `packages/typescript/` has been renamed to `packages/core/` and `packages/native-plugins/` to `packages/native/`.
- Capability tiers (Basic/Extended/Autonomy) are defined in `@elizaos/core`.
...
- **Beta releases**: `develop` is on `2.0.4` with beta tags (latest: `v2.0.3-beta.5`). Pin a specific version in `package.json` rather than tracking `@beta` in production.
