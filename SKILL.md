...
description: >
  ...
  latest tag `v2.0.3-beta.7` as of 2026-06-28) with TypeScript / Python / Rust runtimes, protobuf
  ...
  Last verified against the live repo: 2026-08-31.
---
...
### Branches & Versions (verified 2026-08-31)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.4` | Active beta — this is the v2 line | ~30 packages, 111 plugins, polyglot (TS+Python+Rust) |
...
**Latest published tag:** `v2.0.3-beta.7` (2026-06-28). New beta tags ship from `develop` continuously — install via `bun i -g @elizaos/cli@beta`. There is **no stable v2 release** yet — the project moved from alpha to beta stage.
...
plugins/              → 111 first-party plugins at top level (Discord,
...
- **Beta churn**: `develop` has moved from alpha to beta stage (root `2.0.4`, `@elizaos/core` at `2.0.3-beta.7`). Pin a specific beta version in `package.json` rather than tracking `@beta` in production.
