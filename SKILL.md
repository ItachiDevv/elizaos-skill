...
description: >
  ...
  Covers the v2 (default `develop` branch — currently `2.0.4` in package.json,
  latest tag `v2.0.3-beta.7` as of 2026-06-28) with TypeScript runtime, capability tiers
  ...
  Last verified against the live repo: 2026-06-29.
---
...
### Branches & Versions (verified 2026-06-29)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.4` | Active — v2 line (graduated from alpha to stable semver) | 36 packages, 151 plugins |
...
**Latest published tag:** `v2.0.3-beta.7` (2026-06-28). Beta tags ship from `develop` — install via `bun i -g @elizaos/cli@beta`. The project has graduated from alpha to beta versioning; no stable v2 release has shipped yet.
...
### Monorepo Layout — `develop` branch (verified 2026-06-29)

```
packages/             → 36 packages total (significantly restructured; python/, rust/, schemas/ removed)
  agent/              → Agent runtime composition layer
  app/                → App UI/client
  app-core/           → Shared app logic
  benchmarks/         → Perf benchmarks
  browser-extension/  → Browser extension
  cloud/              → Cloud service layer
  ...
  native/             → Native app support
  os/                 → OS-level integration
  plugin-remote-manifest/ → Remote plugin manifest
  plugin-sub-agent-claude-code/ → Sub-agent Claude Code integration
  ...
  test/               → Test utilities
  training/           → Training/fine-tuning tooling
  tui/                → Terminal UI
  ui/                 → React web dashboard
  vault/              → Secret/key management
  (+ 3 additional packages not returned in directory listing)

plugins/              → 151 first-party plugins at top level (Discord, Telegram,
                        Twitter, Anthropic, OpenAI, Ollama, OpenRouter, Google,
                        MCP, SQL, native-*, and many more)
```

**Key facts to internalize:**
- The monorepo has been significantly restructured: **36 packages** (was 21), **151 plugins** (was 39). Package `typescript/` was renamed to `core/`; new packages include cloud, deploy, tui, vault, native, logger, scripts, contracts, security, etc.
- The Python/Rust SDK packages (`packages/python/`, `packages/rust/`, `packages/schemas/`) are no longer present in the `develop` branch as of 2026-06-29 — the polyglot architecture was retired.
- Plugins are **inside** the monorepo at top-level `plugins/`, not in a separate org.
- Capability tiers (Basic/Extended/Autonomy) remain in the TypeScript runtime.
...
- **Version graduation**: `develop` has moved from alpha to beta/stable semver (`2.0.0-alpha.176` → `2.0.4`; latest published tag `v2.0.3-beta.7` as of 2026-06-28). Install via `bun i -g @elizaos/cli@beta`. Pin a specific version rather than tracking `@alpha` or `@beta` tags in production.
