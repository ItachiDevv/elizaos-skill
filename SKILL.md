...
description: >
  ...
  latest tag `v2.0.0-beta.2` as of 2026-05-17) with TypeScript runtime, capability tiers
  (Basic/Extended/Autonomy), ServiceBuilder API, x402 payments, and Dexter SDK integration.
  Legacy v1 (1.4.4) lives on the `main` branch.
  Last verified against the live repo: 2026-05-18.
---
...
### Branches & Versions (verified 2026-05-18)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.0-beta.0` | Active beta — this is the v2 line | 45 packages, 154+ plugins, TypeScript-focused |
...
**Latest published tag:** `v2.0.0-beta.2` (2026-05-17). Beta releases ship from `develop` — install via `bun i -g @elizaos/cli@beta`. There is **no stable v2 release** yet.

**Recommendation:** For new work target the **`develop` branch (v2 beta)** — it's where all development happens. Only fall back to `main` (1.4.4) if you need a frozen TypeScript-only baseline; upstream development of v1 has stopped.

### Monorepo Layout — `develop` branch (verified 2026-05-18)

```
packages/             → 45 packages total
  ...
  bun-ios-runtime/    → Bun iOS runtime
  checkout-shared/    → Shared checkout components
  cloud-api/          → Cloud API
  cloud-e2e/          → Cloud end-to-end tests
  cloud-frontend/     → Cloud frontend
  cloud-infra/        → Cloud infrastructure
  cloud-routing/      → Cloud routing
  cloud-sdk/          → Cloud SDK
  cloud-services/     → Cloud services
  cloud-shared/       → Cloud shared utilities
  cloud-test-mocks/   → Cloud test mocks
  contracts/          → Smart contracts
  core/               → Core TypeScript SDK (npm: @elizaos/core; was typescript/)
  docs/               → Mintlify docs source (docs.elizaos.ai)
  electrobun-carrots/ → Electrobun framework integration
  elizaos/            → CLI (npm: @elizaos/cli)
  elizaos-setup/      → ElizaOS setup utility
  examples/           → Example agents
  hardware-catalog/   → Hardware catalog
  homepage/           → elizaos.ai marketing site
  ios-native-deps/    → iOS native dependencies
  native-plugins/     → Built-in plugins (sql, bootstrap, etc.)
  os/                 → ElizaOS operating system package
  os-homepage/        → OS homepage
  os-usb-installer/   → OS USB installer
  ...
  shared-brand/       → Shared brand assets
  skills/             → Reusable agent skill library
  steward-session-client/ → Steward session client
  test/               → Test utilities
  training/           → Training data/tools
  ui/                 → React web dashboard
  ui-stories/         → UI component stories
  vault/              → Vault/secrets management
  workflows/          → Workflow definitions

cloud/                → Top-level cloud directory (separate from packages/)

plugins/              → 154+ first-party plugins at top level (Discord,
                        Telegram, Twitter, Solana, EVM, OpenAI, Anthropic,
                        Ollama, OpenRouter, Google, knowledge, MCP, and many more)
```

**Key facts to internalize:**
- The core TypeScript SDK is now at `packages/core/` (was `packages/typescript/`), published as `@elizaos/core`.
- Python and Rust SDKs are **no longer present** in the `develop` branch as of 2026-05-18. The repo has expanded to include iOS/OS packages and cloud infrastructure instead.
...
- Capability tiers (Basic/Extended/Autonomy) remain in the TypeScript SDK.
...
Entity Component System (replaces user system), ServiceBuilder (`createService()`/`defineService()`), formal Event system (30+ EventType enum), Task system (`TaskWorker` + persistent queue), multi-agent orchestration (`ElizaOS` class), model handler registry with priority routing, action chaining (`ActionContext`), working memory, x402 payment types, run tracking, capability tiers, autonomy mode, TypeScript runtime (Python/Rust SDKs no longer in develop branch as of 2026-05-18).

For full v2 details, read **[v2 Architecture](references/v2-architecture.md)**. Note: as of 2026-05-18 the v2 line has graduated from alpha to beta (`2.0.0-beta.0`).

## Quick Start

```bash
bun install -g @elizaos/cli@beta    # @beta = v2 develop line; omit for legacy v1.4.4
...
- **Beta releases**: `develop` is now on the beta track (`2.0.0-beta.0`, latest tag `v2.0.0-beta.2`). Pin a specific beta version in `package.json` rather than tracking `@beta` in production.
