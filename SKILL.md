...
description: >
  ...
  latest tag `v2.0.0-alpha.535` as of 2026-05-02) with capability tiers (Basic/Extended/Autonomy),
  ServiceBuilder API, x402 payments, and Dexter SDK integration.
  Legacy v1 (1.4.4) lives on the `main` branch.
  Last verified against the live repo: 2026-05-04.
---
...
### Branches & Versions (verified 2026-05-04)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.0-alpha.176` | Active alpha — this is the v2 line | 21 packages, 106 plugins |
...
**Latest published tag:** `v2.0.0-alpha.535` (2026-05-02). New alpha tags ship from `develop` continuously — install via `bun i -g @elizaos/cli@alpha`. There is **no stable v2 release** yet — all v2 work has been alpha for 13+ months.
...
### Monorepo Layout — `develop` branch (verified 2026-05-04)
...
packages/
  ...
  vault/              → Secrets and vault management

plugins/              → 106 first-party plugins at top level (Discord,
                        Telegram, Twitter, OpenAI, Anthropic, Ollama,
                        OpenRouter, Google, knowledge, MCP, and more)
```

**Key facts to internalize:**
- **`packages/typescript/` renamed to `packages/core/`** — The core TypeScript SDK is now at `packages/core/` and still publishes as `@elizaos/core`.
- **`packages/python/`, `packages/rust/`, and `packages/interop/` no longer present** on `develop` as of 2026-05-04 — the polyglot runtime packages have been removed from the monorepo.
- Plugins are **inside** the monorepo at top-level `plugins/` (106 as of 2026-05-04), not in a separate `elizaos-plugins` org. The old separate-org claim is obsolete.
...
- **Alpha churn**: `develop` ships a new alpha tag almost daily (alpha.442 → alpha.535 over the last week). Pin a specific alpha version in `package.json` rather than tracking `@alpha` in production.
