...
description: >
  ...
  Covers the v2 beta (default `develop` branch — currently `2.0.0-beta.0` in package.json,
  latest stable tag `v2.0.3` as of 2026-05-20) with TypeScript runtimes, capability tiers
  (Basic/Extended/Autonomy), ServiceBuilder API, x402 payments, and Dexter SDK integration.
  Stable v2 releases (v2.0.0–v2.0.3) shipped in May 2026. Legacy v1 (1.4.4) lives on the `main` branch.
  Last verified against the live repo: 2026-06-15.
---
...
### Branches & Versions (verified 2026-06-15)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.0-beta.0` | Active beta — this is the v2 line | 49 packages, 127+ plugins |
...
**Latest published tag:** `v2.0.3` (2026-05-20). Stable v2 releases began shipping in May 2026 (`v2.0.0` → `v2.0.1` → `v2.0.3`). Install stable via `bun i -g @elizaos/cli`, or beta/pre-release via `bun i -g @elizaos/cli@beta`.
...
### Monorepo Layout — `develop` branch (verified 2026-06-15)

```
packages/             → 49 directories total (major restructuring from v2 alpha)
  agent/              → Agent runtime composition layer
  alberta/            → Alberta agent package
  app/                → Desktop application wrapper
  app-core/           → Shared app logic
  benchmarks/         → Perf benchmarks
  browser-bridge-extension/ → Browser bridge extension
  chip/               → Chip package
  cloud-api/          → Eliza Cloud API backend
  cloud-frontend/     → Eliza Cloud frontend
  cloud-infra/        → Eliza Cloud infrastructure
  cloud-routing/      → Eliza Cloud routing layer
  cloud-sdk/          → Eliza Cloud SDK
  cloud-services/     → Eliza Cloud services
  cloud-shared/       → Shared Cloud utilities
  contracts/          → Smart contract definitions
  core/               → Core TypeScript SDK (npm: @elizaos/core; was typescript/)
  docs/               → Mintlify docs source (docs.elizaos.ai)
  docs-elizacloud-redirect/ → Docs redirect for Eliza Cloud
  ...
  os/                 → OS-level integration
  os-homepage/        → OS homepage
  plugin-host-shim/   → Cross-platform plugin host shim
  ...
  plugin-remote-manifest/ → Remote plugin manifest loader
  plugin-sub-agent-claude-code/ → Sub-agent integration for Claude Code
  plugin-worker-runtime/ → Worker-thread plugin runtime
  prompts/            → Standalone prompt templates
  registry/           → Plugin registry
  research/evidence/  → Research and evidence tooling
  ...
  soc2-verify/        → SOC 2 verification tooling
  sweagent/           → SWE-Agent integration
  test/               → Test utilities
...
plugins/              → 127+ first-party plugins at top level (Discord,
                        Telegram, Twitter, Farcaster, Bluesky, Matrix,
                        Google, Groq, Anthropic, OpenAI, LM Studio,
                        local-inference, MCP, browser, calendar, GitHub,
                        native-* mobile/desktop plugins, and many more)
```

**Key facts to internalize:**
- **Massive growth in plugins/**: 39 → 127+ plugins including native mobile (iOS/Android), desktop, calendar, health, browser, coding tools, and more.
- **Cloud packages added**: `cloud-api`, `cloud-frontend`, `cloud-infra`, `cloud-routing`, `cloud-sdk`, `cloud-services`, `cloud-shared` support the hosted Eliza Cloud platform.
- **Stable v2 shipped**: `v2.0.0`/`v2.0.1`/`v2.0.3` released May 2026; no longer alpha-only.
- **`python/` and `rust/` packages removed** from the `develop` branch as of 2026-06-15 — polyglot SDK claims from v2 alpha no longer apply.
...
- **Version track**: `develop` is on `2.0.0-beta.0`; stable `v2.0.3` was released May 2026. Pin a specific version in `package.json` rather than tracking floating tags in production.
