...
description: >
  ...
  Covers the v2 (default `develop` branch — currently `2.0.0-beta.0` in package.json,
  latest stable tag `v2.0.3` as of 2026-05-20) with capability tiers (Basic/Extended/Autonomy),
  ServiceBuilder API, x402 payments, and Dexter SDK integration. The `main` branch also carries
  `2.0.0-beta.0` following the v2 stable launch; the old v1 (1.4.4) line is archived.
  Last verified against the live repo: 2026-05-25.
---
...
### Branches & Versions (verified 2026-05-25)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.0-beta.0` | Active beta — this is the v2 line | 46 packages, 130+ plugins |
| `main` | `2.0.0-beta.0` | v2 stable line (v1.4.4 era is archived) | Received v2 code post stable launch |
| `v2-develop` | `1.4.4` | **Stale** — identical to old `main`, 3956 commits behind develop | Don't use; migrated into `develop` |
| `v2.0.0` | `2.0.0-alpha` | Separate experimental branch | Different package set incl. `computeruse/`, `sweagent/`, daemon-style packages |

**Latest published tag:** `v2.0.3` (2026-05-20) — the first stable v2 release. Install via `bun i -g @elizaos/cli`. The project moved from alpha to stable between late April and May 2026.

**Recommendation:** For new work target the **`develop` branch (v2 beta)** — it's where all development happens. The `main` branch now carries v2 code as well following the stable launch.

### Monorepo Layout — `develop` branch (verified 2026-05-25)

```
packages/             → 46 entries
  agent/              → Agent runtime composition layer
  alberta/            → (new)
  app/                → Tauri/mobile app wrapper
  app-core/           → Shared app logic
  benchmarks/         → Perf benchmarks
  browser-bridge-extension/ → Browser extension bridge
  chip/               → (new)
  cloud-api/          → Eliza Cloud API package
  cloud-frontend/     → Eliza Cloud frontend
  cloud-infra/        → Eliza Cloud infrastructure
  cloud-routing/      → Eliza Cloud routing
  cloud-sdk/          → Eliza Cloud SDK
  cloud-services/     → Eliza Cloud services
  cloud-shared/       → Shared Cloud utilities
  contracts/          → (new)
  core/               → Core TypeScript SDK (npm: @elizaos/core)
  docs/               → Mintlify docs source (docs.elizaos.ai)
  docs-elizacloud-redirect/ → Cloud docs redirect
  elizaos/            → CLI (npm: @elizaos/cli)
  examples/           → Example agents
  feed/               → Feed package
  homepage/           → elizaos.ai marketing site
  native/             → Native runtime/plugins
  os/                 → OS-level package
  os-homepage/        → OS homepage
  plugin-host-shim/   → Plugin host shim
  plugin-host-shim-android/ → Android plugin host shim
  plugin-host-shim-electrobun/ → Electrobun plugin host shim
  plugin-host-shim-ios/ → iOS plugin host shim
  plugin-remote-manifest/ → Remote plugin manifest
  plugin-sub-agent-claude-code/ → Claude Code sub-agent plugin
  plugin-worker-runtime/ → Plugin worker runtime
  prompts/            → Standalone prompt templates
  robot/              → Robot/automation package
  ...
  soc2-verify/        → SOC2 compliance verification
  test/               → Test utilities
  training/           → Model training utilities
  tui/                → Terminal UI
  ui/                 → React web dashboard
  vault/              → Secrets vault
  workflows/          → Agent workflow utilities

plugins/              → 130+ first-party plugins at top level (Discord,
                        Telegram, Twitter, Farcaster, OpenAI, Anthropic,
                        Ollama, Google GenAI, Groq, MCP, GitHub, Linear,
                        native-device plugins, local inference, and many more)
```
...
- The monorepo grew significantly: packages expanded from 21 → 46, plugins from 39 → 130+, driven largely by cloud infrastructure packages and native-device plugins.
- **v2 is now stable**: `v2.0.3` released 2026-05-20 — the "no stable v2 release" claim is no longer true.
- Capability tiers (Basic/Extended/Autonomy) remain — `AutonomyService` is in the bootstrap plugin.
...
- **Version churn**: `develop` moves fast — the project went from `2.0.0-alpha.176` → `2.0.0-alpha.442` → `v2.0.3` stable between March and May 2026. Pin a specific version in `package.json` for production rather than tracking `@alpha` or `@latest`.
