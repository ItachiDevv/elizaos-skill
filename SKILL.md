...
description: >
  ...
  Covers the v2 beta (default `develop` branch — currently `2.0.4` in package.json,
  latest tag `v2.0.3-beta.7` as of 2026-06-28) with TypeScript runtimes, capability tiers
  ...
  Last verified against the live repo: 2026-07-06.
---
...
### Branches & Versions (verified 2026-07-06)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.4` | Active beta — this is the v2 line | 38 packages, 100+ plugins, TypeScript |
...
**Latest published tag:** `v2.0.3-beta.7` (2026-06-28). Beta tags ship from `develop` continuously — install via `bun i -g elizaos@beta`. The project has moved from alpha to beta channel.
...
### Monorepo Layout — `develop` branch (verified 2026-07-06)

```
packages/             → 38 packages (major restructuring since April 2026)
  ...
  core/               → Core TypeScript SDK (npm: @elizaos/core; renamed from `typescript/`)
  deploy/systemd      → systemd deployment config
  docs/               → Mintlify docs source (docs.elizaos.ai)
  elizaos/            → CLI (npm: @elizaos/cli)
  evidence/           → Evidence / audit trail package
  examples/           → Example agents
  feed/               → Feed package
  homepage/           → elizaos.ai marketing site
  import-conversations/ → Conversation import utilities
  lifeops-bench/      → LifeOps benchmarking
  logger/             → Logging utilities
  native/             → Native OS integration layer
  os/                 → OS-level agent package
  ...
  plugin-worker-runtime/  → Worker runtime plugin
  ...
  vault/              → Secrets/vault package

plugins/              → 100+ first-party plugins at top level (Discord,
                        Telegram, Twitter, Farcaster, OpenAI, Anthropic,
                        Ollama, OpenRouter, Google, knowledge, MCP, native
                        iOS/Android plugins, computeruse, and many more)
```

**Key facts to internalize:**
- **Python and Rust SDKs were removed** from the `develop` branch — `packages/python/` and `packages/rust/` no longer exist. The framework is now TypeScript-only on `develop`.
- The core TypeScript SDK package was renamed from `packages/typescript/` to `packages/core/`.
...
- Versioning has moved from alpha (`2.0.0-alpha.N`) to beta (`2.0.3-beta.N`). Install with `elizaos@beta`.
...
```bash
bun install -g elizaos@beta   # @beta = v2 develop line; omit for legacy v1.4.4
...
## @elizaos/core API Quick Reference (v2 beta — develop branch)

These are the **exact** type signatures for the current beta. Use these when writing plugins.
...
- **Beta churn**: `develop` ships beta tags frequently. Pin a specific beta version in `package.json` rather than tracking `@beta` in production. Versioning moved from `2.0.0-alpha.N` to `2.0.3-beta.N` (latest: `v2.0.3-beta.7` as of 2026-06-28).
