...
description: >
  ...
  Covers the v2 line (default `develop` branch — currently `2.0.4` in package.json,
  ...
  Last verified against the live repo: 2026-08-24.
---
...
### Branches & Versions (verified 2026-08-24)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.4` | Active v2 — this is the main development line | 33 packages, ~125 plugins, TypeScript |
...
**Latest published tag:** `v2.0.3-beta.7` (2026-06-28). Install via `bun i -g @elizaos/cli@beta`. The v2.0.x series was promoted from alpha to beta/stable in May 2026; there is now a v2.0.3 stable release.
...
### Monorepo Layout — `develop` branch (verified 2026-08-24)
...
packages/             → 33 packages
  ...
  synthetic-world/    → Synthetic environment for testing
  test/               → Test utilities
  training/           → Model training utilities
  ui/                 → React web dashboard
  vault/              → Secrets/credentials management

plugins/              → ~125 first-party plugins at top level (Discord,
                        Telegram, Twitter, Solana, EVM, OpenAI, Anthropic,
                        Ollama, OpenRouter, Google, knowledge, MCP, native
                        device plugins, and many more)
```

**Key structural changes since April 2026:**
- `typescript/` renamed to `core/` — npm package remains `@elizaos/core`.
- `native-plugins/` replaced by `native/`.
- Python and Rust SDKs (`packages/python/`, `packages/rust/`) have been removed from the monorepo.
- `interop/`, `schemas/`, `scenario-schema/`, `templates/` removed.
- Plugin count grew from 39 to ~125 with many new native and integration plugins.
...
- **Version progression**: `develop` package.json is now `2.0.4`; latest published tag is `v2.0.3-beta.7` (2026-06-28). The project moved from alpha (`2.0.0-alpha.xxx`) to beta/stable in May 2026. Pin a specific version in `package.json` rather than tracking `@beta` in production.
