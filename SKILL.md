...
description: >
  ...
  Covers the v2 release line (default `develop` branch — currently `2.0.0-beta.0` in package.json,
  latest published tag `v2.0.3` as of 2026-05-20) with TypeScript runtime, capability tiers
  ...
  Last verified against the live repo: 2026-06-01.
---
...
### Branches & Versions (verified 2026-06-01)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.0-beta.0` | Active development — this is the v2 line | 47 packages, 126+ plugins |
...
**Latest published tag:** `v2.0.3` (2026-05-20). v2 has shipped stable releases — install via `bun i -g @elizaos/cli`. Legacy v1 (1.4.4) is on the `main` branch; v2 stable is the recommended install.
...
### Monorepo Layout — `develop` branch (verified 2026-06-01)

```
packages/             → 47 packages total (TypeScript only; Python/Rust SDKs removed)
  agent/              → Agent runtime composition layer
  app/                → Tauri desktop wrapper
  app-core/           → Shared app logic
  benchmarks/         → Perf benchmarks
  cloud-api/cloud-frontend/cloud-infra/cloud-routing/
  cloud-sdk/cloud-services/cloud-shared/ → Eliza Cloud stack
  core/               → Core TypeScript SDK (npm: @elizaos/core; was: typescript/)
  docs/               → Docs source (docs.elizaos.ai)
  ...
  (+ alberta, browser-bridge-extension, chip, contracts, feed, os-homepage,
     plugin-host-shim*, plugin-remote-manifest, plugin-sub-agent-claude-code,
     plugin-worker-runtime, research, robot, scripts, security, soc2-verify,
     test, training, vault, docs-elizacloud-redirect)

plugins/              → 126+ first-party plugins at top level (Discord,
                        Telegram, Twitter, Farcaster, OpenAI, Anthropic,
                        Ollama, Google, Groq, MCP, and many native/mobile
                        plugins added in 2026)
```

**Key facts to internalize:**
- **Python and Rust SDKs have been removed** from `packages/` on `develop` as of 2026.
- Plugins are **inside** the monorepo at top-level `plugins/`, not in a separate `elizaos-plugins` org.
- Plugin count has grown substantially — 126+ directories as of 2026-06-01.
...
- **Stable v2 is shipping**: As of 2026-05-20 `v2.0.3` is the latest stable release — install via `bun i -g @elizaos/cli` (no `@alpha` needed). The `develop` branch is now on `2.0.0-beta.0` for the next release cycle.
