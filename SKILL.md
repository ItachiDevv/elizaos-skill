...
description: >
  ...
  Covers the v2 beta (default `develop` branch — currently `2.0.12-beta.0` in package.json,
  ...
  Last verified against the live repo: 2026-08-17.
---
...
### Branches & Versions (verified 2026-08-17)

| Branch | package.json | Status | Notes |
|--------|---------|--------|---------|
| `develop` (default) | `2.0.12-beta.0` | Active beta — this is the v2 line | ~30 packages, ~127 plugins, TypeScript |
...
**Latest published versioned tag:** `v2.0.3-beta.7` (2026-06-28). New beta tags ship from `develop` continuously — install via `bun i -g @elizaos/cli@beta`. There is **no stable v2 release** yet — all v2 work has been in pre-release (alpha→beta).
...
### Monorepo Layout — `develop` branch (verified 2026-08-17)

```
packages/              → ~30 packages (TypeScript monorepo)
  agent/              → Agent runtime composition layer
  app/                → Tauri desktop wrapper
  app-core/           → Shared app logic
  auth/               → Authentication utilities
  benchmarks/         → Perf benchmarks
  chip/               → Chip/embedded integration
  cloud/              → Cloud services integration
  core/               → Core TypeScript SDK (npm: @elizaos/core) — was typescript/
  corpus-tools/       → Corpus/data tooling
  docs/               → Mintlify docs source (docs.elizaos.ai)
  elizaos/            → CLI (npm: @elizaos/cli)
  elizaresearch/      → Research tooling
  evidence/           → Evidence/CI artifact storage
  examples/           → Example agents
  homepage/           → elizaos.ai marketing site
  logger/             → Logging utilities
  native/             → Built-in plugins (sql, bootstrap, etc.) — was native-plugins/
  os/                 → OS integration layer
  os-homepage/        → OS product homepage
  prompts/            → Standalone prompt templates
  registry/           → Plugin registry
  robot/              → Robotics/automation integration
  scenario-runner/    → Scenario testing harness
  scripts/            → Build and dev scripts
  shared/             → Cross-package shared utilities
  skills/             → Reusable agent skill library
  test/               → Test utilities
  training/           → Model training/fine-tuning tooling
  ui/                 → React web dashboard
  vault/              → Secrets/credentials management

plugins/              → ~127 first-party plugins at top level (Discord,
                        Telegram, OpenAI, Anthropic, SQL, MCP, GitHub,
                        Google Workspace, Slack, Signal, iMessage, etc.)
```

**Key facts (updated 2026-08-17):**
- **Python and Rust SDKs have been removed** from `develop` — `packages/python/` and `packages/rust/` no longer exist; the project is now TypeScript-only.
- **Protobuf schemas and interop layer removed** — `packages/schemas/` and `packages/interop/` are gone.
- Plugins are **inside** the monorepo at top-level `plugins/`, not in a separate `elizaos-plugins` org.
- The plugin count grew from 39 to ~127 with many new native, cloud, and platform integrations.
...
- **Beta churn**: `develop` ships beta tags frequently (moved from alpha.* series to beta channel in 2026). Pin a specific beta version in `package.json` rather than tracking `@beta` in production.
