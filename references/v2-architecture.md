...
Root version: **`2.0.0-beta.0`** in `develop/package.json`; latest published tag **`v2.0.0-beta.2`** (2026-05-17).
Verified against the live repo: 2026-05-18. Node 23+, Bun 1.3.4+.

## Package Layout (verified)

The monorepo is **TypeScript-focused** with first-party plugins inside the repo at top-level `plugins/`. Plugins are no longer in a separate org — the old `elizaos-plugins/` organization claim is obsolete. Note: Python and Rust SDKs previously in `packages/python/` and `packages/rust/` are **no longer present** in the `develop` branch as of 2026-05-18.

### Monorepo Packages (`packages/` — 45 entries on `develop`)
...
### Top-level `plugins/` directory (154+ entries on `develop`)

First-party plugins live inside the monorepo, not a separate org. Includes platform integrations (Discord, Telegram, Twitter, Farcaster, iMessage, Line, Matrix, Nostr), LLM providers (OpenAI, Anthropic, Ollama, OpenRouter, Google, Groq, LM Studio, local inference), chains (Solana, EVM, Hyperliquid), knowledge/RAG, MCP, native mobile/desktop plugins, and many more (154+ total).
...
| First-party plugins | In `packages/plugin-*` | Top-level `plugins/` directory (154+ plugins on `develop`) |
| Runtime languages | TypeScript only | TypeScript (Python + Rust SDKs removed from `develop` as of 2026-05-18) |
