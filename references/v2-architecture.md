...
Branch: **`develop`** (default branch — v2 stable lives here, NOT `v2-develop` which is stale v1).
Root version: **`2.0.0-beta.0`** in `develop/package.json`; latest stable tag **`v2.0.3`** (2026-05-20). **v2 is now stable** — `v2.0.0` shipped May 2026.
Verified against the live repo: 2026-06-08. Node 23+, Bun 1.3.4+.

## Package Layout (verified)

The monorepo is **TypeScript-first** with first-party plugins inside the repo at top-level `plugins/`. Plugins are no longer in a separate org — the old `elizaos-plugins/` organization claim is obsolete. Note: Python and Rust SDK packages that existed in the alpha are no longer present in the monorepo as of 2026-06-08.

### Monorepo Packages (`packages/` — 49 entries on `develop`)

| Package dir | Purpose |
|---------|---------|
| `agent/` | Agent runtime composition layer |
| `alberta/` | (Internal package) |
| `app/` | Tauri desktop wrapper |
| `app-core/` | Shared application logic |
| `benchmarks/` | Performance benchmarks |
| `browser-bridge-extension/` | Browser bridge extension |
| `chip/` | (Internal package) |
...
| `core/` | Core TypeScript SDK (publishes as `@elizaos/core`; replaces `typescript/`) |
...
| `soc2-verify/` | SOC2 verification tooling |
| `sweagent/` | SWE-Agent integration |
| `test/` | Shared test utilities |
| `training/` | Training data and utilities |
| `tui/` | Terminal UI |
| `ui/` | React web dashboard |
| `vault/` | Secure secrets vault |

**Packages removed since April 2026 alpha:** `interop/`, `native-plugins/`, `python/`, `rust/`, `scenario-schema/`, `schemas/`, `templates/`, `typescript/`. The Python and Rust SDK packages are no longer present in the monorepo as of 2026-06-08.

### Top-level `plugins/` directory (116+ entries on `develop`)

First-party plugins live inside the monorepo, not a separate org. Includes platform integrations (Discord, Telegram, Twitter, Farcaster, iMessage, Slack-like services), LLM providers (OpenAI, Anthropic, Google, Groq, LM Studio, local inference), native mobile plugins (camera, calendar, contacts, messages, phone, screen capture, etc.), cloud integrations, and 116+ total entries.
...
| First-party plugins | In `packages/plugin-*` | Top-level `plugins/` directory (116+ plugins on `develop`) |
| Runtime languages | TypeScript only | TypeScript (Python + Rust packages removed from monorepo as of 2026-06) |
