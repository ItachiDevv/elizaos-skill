...
Verified against the live repo: 2026-06-29. Node 23+, Bun 1.3.4+.

## Package Layout (verified)

First-party plugins live inside the repo at top-level `plugins/`. Plugins are no longer in a separate org — the old `elizaos-plugins/` organization claim is obsolete.

> **Note (2026-06-29):** The monorepo was significantly restructured. Python/Rust SDKs (`packages/python/`, `packages/rust/`) and protobuf schemas (`packages/schemas/`) were removed from the `develop` branch. The package count grew from 21 → 36 and the plugin count from 39 → 151.

### Monorepo Packages (`packages/` — 36 entries on `develop`)
...
### Top-level `plugins/` directory (151 entries on `develop`)

First-party plugins live inside the monorepo, not a separate org. Includes platform integrations (Discord, Telegram, Twitter, Farcaster, Signal, Matrix, WhatsApp, Slack, WeChat, Line, Instagram, Bluesky, Nostr), LLM providers (OpenAI, Anthropic, Ollama, OpenRouter, Google, Groq, LM Studio, NearAI), native device plugins (camera, contacts, calendar, filesystem, messages, phone, location, screencapture, etc.), knowledge/RAG, MCP, SQL, and many more.
...
| First-party plugins | In `packages/plugin-*` | Top-level `plugins/` directory (151 plugins on `develop`) |
| Runtime languages | TypeScript only | TypeScript (Python + Rust SDKs removed from `develop` as of 2026-06-29) |
