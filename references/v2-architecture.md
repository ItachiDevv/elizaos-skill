...
Verified against the live repo: 2026-08-24. Node 23+, Bun 1.3.4+.
...
### Monorepo Packages (`packages/` — 33 entries on `develop`, verified 2026-08-24)
...
| `evidence/` | Evidence collection tooling |
| `examples/` | Example agent projects |
| `homepage/` | elizaos.ai marketing site |
| `logger/` | Logging utilities |
| `native/` | Native platform integration (supersedes `native-plugins/`) |
...
**Removed since April 2026:** `interop/`, `native-plugins/` (→ `native/`), `python/`, `rust/`, `scenario-schema/`, `schemas/`, `templates/`, `typescript/` (→ `core/`). Python and Rust SDKs have been removed from the monorepo.

### Top-level `plugins/` directory (~125 entries on `develop`, verified 2026-08-24)

First-party plugins live inside the monorepo, not a separate org. Now ~125 plugins including platform integrations (Discord, Telegram, Twitter, Farcaster), LLM providers (OpenAI, Anthropic, Ollama, OpenRouter, Google), chains (Solana, EVM), knowledge/RAG, MCP, native device/mobile plugins, and many more. Grew from 39 plugins in April 2026.
...
| First-party plugins | In `packages/plugin-*` | Top-level `plugins/` directory (~125 plugins on `develop`) |
| Runtime languages | TypeScript only | TypeScript (Python and Rust SDKs removed from monorepo) |
