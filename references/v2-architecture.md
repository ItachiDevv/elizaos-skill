...
Root version: **`2.0.4`** in `develop/package.json`; latest published tag **`v2.0.3-beta.11`** (2026-07-16).
Verified against the live repo: 2026-07-27. Node 23+, Bun 1.3.4+.

## Package Layout (verified 2026-07-27)

The monorepo is **TypeScript-based** with first-party plugins inside the repo at top-level `plugins/`. The Python and Rust SDK packages that existed in the v2 alpha are no longer present in `develop`. Plugins are no longer in a separate org — the old `elizaos-plugins/` organization claim is obsolete.

### Monorepo Packages (`packages/` — 39 entries on `develop`)
...
### Top-level `plugins/` directory (100+ entries on `develop`)

First-party plugins live inside the monorepo, not a separate org. Includes platform integrations (Discord, Telegram, Twitter, Farcaster, iMessage, Matrix, Line, Feishu), LLM providers (OpenAI, Anthropic, Ollama, Groq, Google GenAI, LM Studio), native device plugins (camera, contacts, filesystem, calendar, location, system, etc.), and more.
