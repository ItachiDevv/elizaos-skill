...
Branch: **`develop`** (default branch — v2 lives here, NOT `v2-develop` which is stale v1).
Root version: **`2.0.0-beta.0`** in `develop/package.json`; latest published tag **`v2.0.3`** (2026-05-20).
Verified against the live repo: 2026-06-01. Node 23+, Bun 1.3.4+.
...
| `core/` | Core TypeScript SDK (publishes as `@elizaos/core`; was `typescript/`) |
...
| `plugin-host-shim/` | Plugin host shim (web) |
| `plugin-host-shim-android/` | Plugin host shim (Android) |
| `plugin-host-shim-electrobun/` | Plugin host shim (Electrobun) |
| `plugin-host-shim-ios/` | Plugin host shim (iOS) |
...
> **Note:** `python/`, `rust/`, `schemas/`, `interop/`, `native-plugins/`, `templates/`, `scenario-schema/` have been removed from `packages/` since 2026-04-28. The TypeScript SDK package directory was renamed from `typescript/` to `core/`.

### Top-level `plugins/` directory (126+ entries on `develop`, verified 2026-06-01)

First-party plugins live inside the monorepo, not a separate org. Includes platform integrations (Discord, Telegram, Twitter, Farcaster, Bluesky, Matrix, Line, Feishu), LLM providers (OpenAI, Anthropic, Ollama, Google GenAI, Groq, LM Studio, NearAI), native/mobile plugins (contacts, calendar, camera, desktop, llama, messages, phone, screencapture, etc.), and many more. The plugin count has grown from 39 (April 2026) to 126+ (June 2026).
...
| First-party plugins | In `packages/plugin-*` | Top-level `plugins/` directory (126+ plugins on `develop`) |
| Runtime languages | TypeScript only | TypeScript (Python + Rust SDKs removed from monorepo by 2026) |
